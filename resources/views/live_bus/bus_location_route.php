...
var detail=location_value;
  ...
  map = new google.maps.Map(document.getElementById("map"), {
    zoom: zoom_value,
    center: { lat: parseFloat(bus_location_focus_lat), lng: parseFloat(bus_location_focus_long) },
  });
       image = {
   ...
   var bus_data = detail;
    var bus_data_explode=bus_data.split(","); 
      ...
      position: { lat: parseFloat(bus_data_explode[1]), lng: parseFloat(bus_data_explode[2]) },
      map,
      icon: image,
      shape: shape,
...
function set_location(s_no){
    ...
    $.ajax({
type:"POST",
url:access_link+"live_bus/bus_location_route_location_ajax.php?s_no="+s_no+"&location_date="+location_date,
...
if(lat_old==0){
    ...
    total_distance=Number(total_distance)+Number(distance(Number(lat_old), Number(long_old), Number(bus_location_focus_lat), Number(bus_location_focus_long), "K"));
     lat_old=bus_location_focus_lat;
    long_old=bus_location_focus_long;
...
if(total_distance<0.1){
   ...
   map = new google.maps.Map(document.getElementById("map"), {
    zoom: zoom_value,
    center: { lat: parseFloat(bus_location_focus_lat1), lng: parseFloat(bus_location_focus_long1) },
  });
  ...
  image = {
   ...
   shape = {
    ...
    type: "poly",
	     ...
	     new google.maps.Marker({
      position: { lat: parseFloat(bus_location_focus_lat), lng: parseFloat(bus_location_focus_long) },
      map,
      icon: image,
      shape: shape,
...
function initMap() {
 ...
 <li><a href="javascript:get_content('live_bus/bus_tracking')"><i class="fa fa-dashboard"></i>Live Bus</a></li>
                ...
                <div class="col-md-12">
                    <div class="form-group">
						  <label>Location Date<font style="color:red"><b>*</b></font></label>
						   <input type="date"  name="location_date" value="2022-12-05" class="form-control" required>
						</div>
                </div>
                <div class="col-md-12">
<div class="col-md-12">
        <input type="hidden" id="location_data_hidden3" value="BHARAT BENZ,,1">
<center><button type="button" style="width:100%;margin:10px" class="btn btn-primary"  Title="Bus Location Not Set" disabled  onclick="get_location('3');" id="bus_id_3">BHARAT BENZ[MH19TS0001]<small></small></button></center>
	</div>
